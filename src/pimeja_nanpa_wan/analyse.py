...
def percentages(svg_path: Path) -> dict[str, float]:
    ...
    for line in histogram.splitlines():
        ...
        if not match:
            continue
        pixels, red, green, blue, alpha = (int(part or 255) for part in match.groups())
        family = named_colour(red, green, blue)
        if pixels >= MINIMUM_COLOUR_PIXELS and alpha >= 128 and (not allowed_families or family in allowed_families):
            count[family] += pixels
...
def interpolate(hex_colour: str, fraction: float) -> str:
    fraction = max(0.0, min(1.0, fraction / 100))
    target = tuple(int(hex_colour[index : index + 2], 16) for index in (1, 3, 5))
    rgb = tuple(round(255 + (channel - 255) * fraction) for channel in target)
...
def make_maps(rows: list[dict[str, str]]) -> None:
    ...
    for colour, display in COLOURS.items():
        ...
        for feature in geojson["features"]:
            ...
            iso3 = props.get("ISO_A3") or props.get("ISO3166-1-Alpha-3") or props.get("iso_a3")
            if iso3 == "-99":
                iso3 = {"France": "FRA", "Norway": "NOR"}.get(props.get("name"), iso3)
            ...
            paths.append(f'<path d="{d}" fill="{interpolate(display, value)}"/>')
  ...
  <desc id="desc">Each UN member is shaded from white (0%) to {escape(colour)} (100%) according to the share of its flag classified as {escape(colour)}.</desc>
  <rect width="720" height="360" fill="#dcecf5"/>
...
def main() -> None:
    ...
    parser.add_argument("--start", type=int, default=0, help="zero-based manifest index")
    parser.add_argument("--count", type=int, default=193, help="number of flags to analyse")
    parser.add_argument("--finalize", action="store_true", help="assemble CSV/maps from cached rows only")
    ...
    if not args.finalize:
        selection = list(enumerate(manifest[args.start : args.start + args.count], start=args.start + 1))
        print(f"Analysing {args.start + 1}–{args.start + len(selection)} of 193...", flush=True)
        with ThreadPoolExecutor(max_workers=4) as executor:
            futures = [executor.submit(analyse_one, index, country) for index, country in selection]
    ...
    missing = [country["iso3"] for country in manifest if not (CACHE / f"{country['iso3']}.json").exists()]
    if missing:
